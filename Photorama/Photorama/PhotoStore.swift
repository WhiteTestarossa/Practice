...
class PhotoStore {
    ...
    func fetchInterestingPhotos(completion: @escaping (Result<[Photo], Error>) -> Void)  {
        ...
        let task = session.dataTask(with: request) { data, response, error in
            ...
            if case .success = result {
                do {
                    try self.persisntentContainer.viewContext.save()
                } catch {
    ...
    private func processPhotosRequest(data: Data?, error: Error?) -> Result<[Photo], Error> {
        ...
        let context = persisntentContainer.viewContext
    ...
    private func processImageRequest(data: Data?, error: Error?) -> Result<UIImage, Error> {
        guard let imageData = data, let image = UIImage(data: imageData) else { // Couldn't create an image
                if data == nil {
                    return .failure(error!)
                } else {
    ...
    func fetchAllPhotos(completion: @escaping (Result<[Photo], Error>) -> Void) {
        ...
        let viewcontext = persisntentContainer.viewContext
        viewcontext.perform {
            do {
